...
async def ask_sayori(prompt):
  ...
  system_instruction = (
      ...
      'Phong cách trò chuyện ngọt ngào, ấm áp, đôi khi có những hành động thân'
      ' mật, ôm ấp, làm nũng hoặc những biểu cảm gợi cảm nhẹ nhàng, lãng mạn'
      ' (soft NSFW) nhưng vẫn giữ được nét đáng yêu, trong sáng đặc trưng của'
      ' Sayori. '
  ...
  for i in range(len(API_KEYS)):
    ...
    try:
      ...
      if '429' in err_msg or 'RESOURCE_EXHAUSTED' in err_msg:
        ...
        return (
            '*nghiêng đầu bối rối* Hì hì, hệ thống của tớ đang gặp chút xíu lỗi'
            f' rồi: {err_msg}'
        )
  ...
  def from_data(cls, data):
    ...
    return cls(
        discord.FFmpegPCMAudio(
            filename, executable=ffmpeg_bin, **FFMPEG_OPTIONS
        ),
        data=data,
    )
...
def play_next(ctx):
  ...
  if guild_id in music_queues and music_queues[guild_id]:
    ...
    async def run_autoplay():
      await ctx.send(
          '🔄 *SoundCloud Autoplay*: Tớ đang tự tìm bài hát liên quan đến'
      ...
      if auto_track and ctx.voice_client:
        ...
        await ctx.send(
            '📻 *Autoplay SoundCloud*: Đang phát bài gợi ý tiếp theo:'
...
async def play_music(ctx, *, search: str):
  ...
  if ctx.voice_client is None:
    try:
      ...
      return await ctx.send(
          f'*bĩu môi* Tớ không vào được phòng thoại... Lỗi: `{e}`'
      )
  ...
  async with ctx.typing():
    try:
      ...
      return await ctx.send(
          f'*nghiêng đầu* Tớ không lấy được dữ liệu từ link này rồi... Lỗi:'
          f' `{e}`'
      )
...
async def show_queue(ctx):
  ...
  if queue:
    ...
    for idx, item in enumerate(queue[:10], start=1):
      title = (
          item.get('title', 'Bài hát SoundCloud')
          if isinstance(item, dict)
          else item.title
      )
      uploader = (
          item.get('uploader', 'Nghệ sĩ')
          if isinstance(item, dict)
          else item.uploader
      )
    ...
    msg += (
        '*Hàng chờ hiện tại đã hết! (SoundCloud Autoplay sẽ tự phát bài gợi ý'
        ' tiếp theo)*'
    )
...
async def toggle_autoplay(ctx):
  ...
  await ctx.send(
      '📻 Tính năng **SoundCloud Autoplay (Tự phát bài liên quan)** hiện đã:'
      f' **{status_str}**'
  )
