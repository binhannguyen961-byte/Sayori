...
# --- 2. Cấu hình Google GenAI SDK Mới (Sayori AI) ---
API_KEYS = []
    ...
    if 'on.soundcloud.com' in url:
        try:
            req = urllib.request.Request(
                url, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'}
            )
...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def fetch_info(cls, query, loop=None):
        ...
        if query.startswith('http://') or query.startswith('https://'):
            target_url = await loop.run_in_executor(None, lambda: resolve_soundcloud_url(query))
        else:
...
async def on_ready():
    ...
    await sayori_bot.change_presence(
        activity=discord.Game(name="Mở SoundCloud Playlist & Autoplay cùng cậu! ☁️🎶")
    )
...
# --- 5. Logic Autoplay SoundCloud & Chuyển bài ---
        ...
        if entries:
            ...
            if 'url' not in chosen_data or not chosen_data['url'].startswith('http'):
                chosen_data = await loop.run_in_executor(
                    None, lambda: ytdl.extract_info(chosen_data.get('webpage_url', chosen_data.get('url')), download=False)
                )
...
def play_next(ctx):
    ...
    if guild_id in music_queues and music_queues[guild_id]:
        ...
        coro = ctx.send(f"☁️ *nhún nhảy theo SoundCloud* Đang phát bài tiếp theo: **{next_source.title}** - `{next_source.uploader}` 🎶")
        ...
        async def run_autoplay():
            await ctx.send(f"🔄 *SoundCloud Autoplay*: Tớ đang tự tìm bài hát liên quan đến **{last_track.title}** cho cậu nè...")
            ...
            if auto_track and ctx.voice_client:
                ...
                await ctx.send(f"📻 *Autoplay SoundCloud*: Đang phát bài gợi ý tiếp theo: **{auto_track.title}** - `{auto_track.uploader}` ☁️✨")
            else:
...
async def play_music(ctx, *, search: str):
    if not ctx.author.voice:
        return await ctx.send("*xoa xoa tay* Cậu vào Voice Channel trước đi rồi tớ mở SoundCloud cho nghe nha!")

    if ctx.voice_client is None:
        try:
            await ctx.author.voice.channel.connect()
        except Exception as e:
            return await ctx.send(f"*bĩu môi* Tớ không vào được phòng thoại... Lỗi: `{e}` (Cậu kiểm tra file `requirements.txt` có `PyNaCl` chưa nhé!)")
    ...
    async with ctx.typing():
        try:
            ...
            return await ctx.send(f"*nghiêng đầu* Tớ không lấy được dữ liệu từ link này rồi... Lỗi: `{e}`")

        if not data:
            return await ctx.send("*bĩu môi* Không tìm thấy bài hát SoundCloud nào cả cậu ơi...")
        ...
        try:
            ...
            if 'entries' in data and data['entries']:
                ...
                await ctx.send(f"☁️ **ĐÃ THÊM PLAYLIST:** `{playlist_title}` với **{len(entries)}** bài vào danh sách chờ!")

                if not ctx.voice_client.is_playing():
                    last_played_track[guild_id] = first_track
                    ctx.voice_client.play(first_track, after=lambda e: play_next(ctx))
                    await ctx.send(f"🎶 *Đang phát bài đầu tiên:* **{first_track.title}** - `{first_track.uploader}` ☀️")

            # Trường hợp Single Track
            else:
                track_data = data['entries'][0] if ('entries' in data and data['entries']) else data
                ...
                if ctx.voice_client.is_playing():
                    ...
                    await ctx.send(f"🎶 *Đang phát SoundCloud:* **{track.title}** - `{track.uploader}` ☀️")
        except Exception as e:
            await ctx.send(f"*ôm đầu bối rối* Lỗi phát nhạc rồi cậu ơi: `{e}`.")
...
async def show_queue(ctx):
    ...
    if queue:
        ...
        for idx, item in enumerate(queue[:10], start=1):
            title = item.get('title', 'Bài hát SoundCloud') if isinstance(item, dict) else item.title
        ...
        msg += "*Hàng chờ hiện tại đã hết! (SoundCloud Autoplay sẽ tự phát bài gợi ý tiếp theo)*"
...
async def toggle_autoplay(ctx):
    ...
    await ctx.send(f"📻 Tính năng **SoundCloud Autoplay (Tự phát bài liên quan)** hiện đã: **{status_str}**")

@sayori_bot.command(name='skip', aliases=['qua'])
async def skip_music(ctx):
    if ctx.voice_client and ctx.voice_client.is_playing():
        ctx.voice_client.stop()
        await ctx.send("*gật đầu* Đã bỏ qua bài hiện tại! Để tớ chuyển bài tiếp theo nha~")
    else:
...
async def stop_music(ctx):
    ...
    if ctx.voice_client and ctx.voice_client.is_playing():
        ctx.voice_client.stop()
        await ctx.send("*ôm lấy tay cậu* Đã tắt nhạc SoundCloud rồi nè. Cần nghe lại thì bảo tớ nha!")
    else:
